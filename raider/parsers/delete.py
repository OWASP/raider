...
def add_delete_parser(parser) -> None:
    ...
    delete_parser.add_argument(
        "project", help="Project name to delete")
    delete_parser.add_argument(
...
def run_delete_command(args):
    ...
    if os.path.isdir(project_dir):
        if os.listdir(project_dir):
            raider.logger.warning("Project \"%s\" contains hyfiles at %s.", args.project, project_dir)
            answer = input(colored_text("Are you sure you want to delete it? (Y/N) ", "RED-BLACK-B"))
            if answer[0].upper() == "Y":
                raider.logger.warning("Deleting %s with all hyfiles inside.", project_dir)
