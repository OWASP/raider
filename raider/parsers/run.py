...
def add_run_parser(parser) -> None:
    run_parser = parser.add_parser(
        "run", help="Run flow or flowgraph"
    )
    run_parser.add_argument("project", nargs="?", help="Project name")
    run_parser.add_argument("--flow", help="Run a single Flow")
    run_parser.add_argument("--graph", help="Run a series of Flows in the FlowGraph until no NextStage is found")
