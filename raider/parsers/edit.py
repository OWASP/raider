...
from raider.utils import list_hyfiles, list_projects, get_project_dir
...
def add_edit_parser(parser) -> None:
    ...
    edit_parser.add_argument(
        "hyfile", help="hyfile name to edit"
    )
...
def run_edit_command(args):
    ...
    if not os.path.isfile(filepath):
        raider.logger.warning("File "
                              + filepath
                              + " doesn't exist. Creating new.")
